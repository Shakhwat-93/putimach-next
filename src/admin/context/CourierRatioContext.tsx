...
export const CourierRatioProvider = ({ children }) => {
  ...
  const [ratios, setRatios] = useState(() => {
    try {
      const cached = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem(CACHE_KEY);
  ...
  useEffect(() => {
    ...
    try {
      (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem(CACHE_KEY, JSON.stringify(ratios));
  ...
  const fetchRatio = async (phone) => {
    let lastError = '';
    try {
      const res = await fetch('/admin-api/courier-ratio-check', {
        method: 'POST',
        headers: {
