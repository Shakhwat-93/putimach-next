...
export const ActiveUsers = () => {
  ...
  useEffect(() => {
    ...
    const raw = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem('presence_last_seen_map');
    const map = raw ? JSON.parse(raw) : {};
    onlineUsers.forEach((u) => {
      map[u.id] = u.online_at || new Date().toISOString();
    });
    (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem('presence_last_seen_map', JSON.stringify(map));
  }, [onlineUsers]);
