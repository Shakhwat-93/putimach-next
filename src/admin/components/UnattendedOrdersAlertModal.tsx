...
export const UnattendedOrdersAlertModal = () => {
  ...
  const [snoozeUntil, setSnoozeUntil] = useState(() => {
    try {
      const stored = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem('of_unattended_snooze_until');
  ...
  const [soundMuted, setSoundMuted] = useState(() => {
    return (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem('of_unattended_sound_muted') === 'true';
  });
  ...
  const toggleSound = () => {
    setSoundMuted(prev => {
      const next = !prev;
      (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem('of_unattended_sound_muted', String(next));
      return next;
    });
  };
  ...
  const handleSnooze = (minutes = 10) => {
    ...
    try {
      (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem('of_unattended_snooze_until', String(until));
    } catch (e) {
