...
export const TaskBoard = () => {
  ...
  useEffect(() => {
    ...
    const checkTime = () => {
      ...
      if (hours >= 22) { // Past 10:00 PM (22:00)
        const dismissedOverdue = (typeof window !== 'undefined' ? sessionStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem(`dismissed_overdue_${todayStr}`);
        if (!dismissedOverdue) {
          setShowOverdueAlert(true);
          setShowReminderAlert(false);
        }
      } else if (hours >= 20) { // Past 8:00 PM (20:00 to 22:00)
        const dismissedReminder = (typeof window !== 'undefined' ? sessionStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem(`dismissed_reminder_${todayStr}`);
  ...
  if (loading && !assignedTasks?.length && !myDailyTasks?.length) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        <Loader2 size={36} className="animate-spin text-primary" />
        <p className="text-sm font-semibold text-muted-foreground animate-pulse">Loading Tasks Workspace...</p>
      </div>
    );
  }
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '12px' }}>
                <button
                  ...
                  onClick={() => {
                    setShowReminderAlert(false);
                    const todayStr = new Date().toDateString();
                    (typeof window !== 'undefined' ? sessionStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem(`dismissed_reminder_${todayStr}`, 'true');
                  }}
                >
                  Later
                </button>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '12px' }}>
                <button
                  ...
                  onClick={() => {
                    setShowOverdueAlert(false);
                    const todayStr = new Date().toDateString();
                    (typeof window !== 'undefined' ? sessionStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem(`dismissed_overdue_${todayStr}`, 'true');
                  }}
                >
                  Dismiss
                </button>
