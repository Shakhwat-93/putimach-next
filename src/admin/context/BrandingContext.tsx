...
const DEFAULT_APP_NAME = 'PutiMach Admin';
...
export const BrandingProvider = ({ children }) => {
  const [branding, setBranding] = useState(() => {
    try {
      const raw = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem(BRANDING_STORAGE_KEY);
  ...
  const persistBranding = useCallback((nextBranding) => {
    ...
    (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem(BRANDING_STORAGE_KEY, JSON.stringify(normalized));
    return normalized;
  }, []);
