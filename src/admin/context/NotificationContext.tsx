...
export const NotificationProvider = ({ children }) => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (!hasShownInitialUnreadToastsRef.current) {
        const lastShown = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem('last_unread_modal_shown_day');
        ...
        if (lastShown !== todayStr) {
          ...
          if (initialUnread.length > 0) {
            ...
            (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem('last_unread_modal_shown_day', todayStr);
          }
        }
  ...
  const clearAllNotifications = async () => {
    try {
      ...
      (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem('notifs_cleared_at', new Date().toISOString());
