...
export const api = {
  ...
  getToyBoxProductNameColumnState() {
    ...
    const cached = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem('of_toybox_pname_state');
  ...
  setToyBoxProductNameColumnState(val) {
    ...
    (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem('of_toybox_pname_state', String(val));
  },
  ...
  async getToyBoxInventory() {
    try {
      ...
      console.warn('getToyBoxInventory fallback:', err);
      return [];
    }
  },
  ...
  async updateOrderStatus(orderId, newStatus, noteText = '', userId = null, userName = '') {
    ...
    if (oldData && typeof this.adjustOrderStock === 'function') {
      try {
        await this.adjustOrderStock(orderId, oldData.status, newStatus, userName || 'System');
      } catch (stockErr) {
        console.warn('Stock adjustment warning:', stockErr);
      }
    }
  ...
  async getSteadfastStatus(orderId, trackingCode) {
    const response = await fetch('/admin-api/courier-status', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const data = await response.json();
    if (!response.ok) {
      const errorMsg = data.error || 'Status sync failed';
      console.error('Steadfast Status Error:', errorMsg);
