...
export const TaskProvider = ({ children }) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      const [daily, completions, assigned] = await Promise.all([
        api.getDailyTasks().catch(err => { console.warn('Daily tasks fetch notice:', err); return []; }),
        api.getDailyCompletions().catch(err => { console.warn('Completions fetch notice:', err); return []; }),
        api.getAssignedTasks(userId, isAdmin).catch(err => { console.warn('Assigned tasks fetch notice:', err); return []; })
      ]);
      setDailyTasks(daily || []);
  ...
  useEffect(() => {
    ...
    const checkDeadlines = async () => {
      ...
      const notifiedStr = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem(`notified_deadlines_${userId}`) || '{}';
      ...
      if (hasNewNotif) {
        (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem(`notified_deadlines_${userId}`, JSON.stringify(notified));
      }
    };
