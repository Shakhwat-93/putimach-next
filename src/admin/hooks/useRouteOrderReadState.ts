...
const readStoredState = (storageKey) => {
  try {
    const raw = (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem(storageKey);
...
const persistStoredState = (storageKey, nextState) => {
  try {
    ...
    (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem(storageKey, JSON.stringify(Object.fromEntries(entries)));
  } catch {
