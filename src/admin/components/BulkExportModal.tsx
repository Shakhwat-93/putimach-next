...
export const BulkExportModal = ({
  ...
  const [history, setHistory] = useState(() => {
    try { return JSON.parse((typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).getItem(EXPORT_HISTORY_KEY)) || []; }
    catch { return []; }
  });
  ...
  useEffect(() => {
    try { (typeof window !== 'undefined' ? localStorage : { getItem:()=>null, setItem:()=>{}, removeItem:()=>{} }).setItem(EXPORT_HISTORY_KEY, JSON.stringify(history.slice(0, 20))); }
    catch {}
  }, [history]);
